...
def dianyingfenlei_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            __loginUserColumn__= dianyingfenlei.__loginUserColumn__
        except:
        ...
        eval('''dianyingfenlei.objects.filter({}='{}').update({}='{}')'''.format(username_str,username,password_str,init_pwd))
...
def dianyingfenlei_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是":
            ...
            if tablename!="users" and 'dianyingfenlei'[:7]!='discuss'and "userid" in dianyingfenlei.getallcolumn(dianyingfenlei,dianyingfenlei):
                req_dict["userid"] = request.session.get("params").get("id")
        ...
        msg['data']['pageSize']  =dianyingfenlei.page(dianyingfenlei, dianyingfenlei, req_dict, request)
...
def dianyingfenlei_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __browseClick__=="是"  and  "clicknum"  in dianyingfenlei.getallcolumn(dianyingfenlei,dianyingfenlei):
            try:
...
def dianyingfenlei_importExcel(request):
    if request.method in ["POST", "GET"]:
        ...
        if file_type in ['xlsx', 'xls']:
            ...
            try:
                for row in range(1, rows):
                    ...
                    req_dict = {}
                    dianyingfenlei.createbyreq(dianyingfenlei, dianyingfenlei, req_dict)
...
def dianyingfenlei_sendemail(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
...
def dianyingfenlei_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select inteltype from storeup where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'dianyingfenlei' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from dianyingfenlei where $intelRecomColumn in ('%s"%("','").join(leixing)+"') union all select * from dianyingfenlei where $intelRecomColumn not in('%s"%("','").join(leixing)+"')")
...
def dianyingfenlei_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM dianyingfenlei {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d')".format(xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM dianyingfenlei {2} GROUP BY DATE_FORMAT({0}, '%Y-%m')".format(xColumnName, yColumnName, where, '%Y-%m')

        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM dianyingfenlei {2} GROUP BY DATE_FORMAT({0}, '%Y')".format(xColumnName, yColumnName, where, '%Y')
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def dianyingfenlei_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        
        where = ' where 1 = 1 '
        
        sql = "SELECT {0}, sum({1}) AS total FROM dianyingfenlei {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def dianyingfenlei_alipay(request):
    if request.method in ["POST", "GET"]:
        alipay = AliPay(
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/djangouops9/dianyingfenlei/notify',
            #notify_url=''
        )
...
def dianyingfenlei_notify(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
        out_trade_no = req_dict['out_trade_no']
        cursor = connection.cursor()
        
        return redirect('http://localhost:8080/djangouops9/admin/dist/index.html#/dianyingfenlei')
